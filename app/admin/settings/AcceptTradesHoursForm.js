...
const AcceptTradesHoursForm = ({ UpdateAcceptTradesHours, currentStartingHour, currentEndingHour }) => {
  const [startingHour, setStartingHour] = useState("");
  const [endingHour, setEndingHour] = useState("");

  const Update = async () => {
    const response = await UpdateAcceptTradesHours({ startingHour, endingHour });
  ...
  return (
    <div className="flex flex-col gap-2 border border-gray-300 p-4 rounded-lg">
      <div className="text-center text-gray-500">
        Accept Trades: {currentStartingHour}:00 - {currentEndingHour}:00
      </div>
...
export default AcceptTradesHoursForm;

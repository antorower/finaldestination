...
const TradeItem = ({ ChangeTradeStatus, tradeId, userId, account, priority, openDate, openTime, status }) => {
  return (
    <div className={`grid grid-cols-12 border ${priority === "high" ? "bg-orange-100" : "bg-gray-100"} w-full`}>
      <RejectTradeButton ChangeTradeStatus={ChangeTradeStatus} tradeId={tradeId} userId={userId} status={status} />
      ...
      <AcceptTradeButton ChangeTradeStatus={ChangeTradeStatus} tradeId={tradeId} userId={userId} status={status} />
    </div>
  );
};

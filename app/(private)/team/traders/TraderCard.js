...
const TraderCard = ({ trader }) => {
  ...
  return (
    <div className={`rounded-xl w-full max-w-[400px] bg-gray-50 text-gray-700 border border-gray-300 px-4 py-2`}>
      <div className={`p-4 border-b border-gray-300 rounded-t-lg `}>
        <div className="flex gap-4 items-center justify-between">
          <Link href={`/team/trader/${trader._id.toString()}`} className="flex items-center justify-center gap-2 text-xl font-semibold">
            <div>{trader.firstName}</div>
            <div>{trader.lastName}</div>
          </Link>
          <Link href={`/team/trader/${trader._id.toString()}`}>👀</Link>
        </div>
      </div>
